const courseData = [
    ...
    },
    {
        ...
        user: {
            ...
            avatar_url:
                'https://files.fullstack.edu.vn/f8-prod/user_avatars/18159/62c30204628ec.png',
            cover_url: '',
            is_comment_blocked: false,
            is_blocked: false,
        },
...
//? Casch 1
/* function Course({ title, description, image, studentsCount }) {
    return (
    ...
    <div className="courses">
        {courseData.map((courses) => (
            <Course 
                key={courses.id}
                title={courses.title}
                description={courses.slug}
                image={courses.thumbnail_url}
                studentsCount={courses.reactions_count}
            />
        ))}
    </div>
)
...
/* function Course({ course }) {
    return (
        <div className="course-item">
            <h2 onClick={() => alert(course.title)}>{course.title}</h2>
            <p>{course.slug}</p>
            <img src={course.thumbnail_url} />
            <p>{course.reactions_count}</p>
        </div>
    )
}
